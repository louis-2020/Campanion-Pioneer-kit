...
uint8_t ADC_4_initVar = 0u;
uint8_t ADC_4_selected = 0u; /* 0 if no configuration selected. 1 otherwise. */
uint32_t ADC_4_currentConfig = 0u; /* Currently active configuration */

/*******************************************************************************
* Function Name: ADC_4_Start
****************************************************************************//**
*
...
*
* \globalvars
*  \ref ADC_4_initVar (RW)
*
*******************************************************************************/
void ADC_4_Start(void)
{
    if (ADC_4_INIT_VAR_INIT_FLAG != (ADC_4_INIT_VAR_INIT_FLAG & ADC_4_initVar))
    {
        ADC_4_Init();
        ADC_4_initVar |= ADC_4_INIT_VAR_INIT_FLAG;
    }
...
/*******************************************************************************
* Function Name: ADC_4_StartEx
****************************************************************************//**
*
* \brief This function starts the ADC_4 and sets the Interrupt
...
void ADC_4_StartEx(cy_israddress userIsr)
{
    ...
    #ifdef ADC_4_IRQ__INTC_CORTEXM4_ASSIGNED
    #if (CY_CPU_CORTEX_M4)
        (void)Cy_SysInt_Init(&ADC_4_IRQ_cfg, userIsr);
        NVIC_EnableIRQ(ADC_4_IRQ_cfg.intrSrc);
    #endif
    #endif

    #ifdef ADC_4_IRQ__INTC_CORTEXM0P_ASSIGNED
...
/* ****************************************************************************
* Function Name: ADC_4_InitConfig
****************************************************************************//*
*
* \brief Configures all of the registers for a given configuration for scanning.
*
* \param scan Number of scan defined in the ADC_4.
*
* \return None
*
* \sideeffect None
...
void ADC_4_InitConfig(const ADC_4_CONFIG_STRUCT *config)
{
    ...
    /* If there is an internal SAR clock, set up its divider values. */
        ...
        ADC_4_intSarClock_Disable();
        ADC_4_intSarClock_SetDivider(config->clkDivider);
        ADC_4_intSarClock_Enable();
    #endif /* ADC_4_CLOCK_INTERNAL */

    /* Init SAR and MUX registers */
    (void)Cy_SAR_DeInit(ADC_4_SAR__HW, deInitRouting);
    (void)Cy_SAR_Init(ADC_4_SAR__HW, config->hwConfigStc);

    /* Connect Vminus to VSSA when even one channel is single-ended or multiple channels configured */
    if(1uL == ADC_4_MUX_SWITCH0_INIT)
    {
        Cy_SAR_SetVssaVminusSwitch(ADC_4_SAR__HW, CY_SAR_SWITCH_CLOSE);

        /* Set MUX_HW_CTRL_VSSA in MUX_SWITCH_HW_CTRL when multiple channels enabled */
        if(1uL < config->numChannels)
        {
            Cy_SAR_SetVssaSarSeqCtrl(ADC_4_SAR__HW, CY_SAR_SWITCH_SEQ_CTRL_ENABLE);
        }
    }
...
/* ****************************************************************************
* Function Name: ADC_4_SelectConfig
****************************************************************************//*
*
* \brief Selects the predefined configuration for scanning.
*
* \param config Number of configuration in the ADC_4.
*
* \param restart Set to 1u if the ADC_4 should be  restarted after
...
void ADC_4_SelectConfig(uint32_t config, uint32_t restart)
{
    ...
    if(ADC_4_TOTAL_CONFIGS > config)
    {
        ...
        if(0u == ADC_4_initVar)
        {
            ...
            ADC_4_vrefAMux_DisconnectAll();
        #endif

        ADC_4_InitConfig(&ADC_4_allConfigs[config]);

        #if (ADC_4_VREF_ROUTED)
            ADC_4_vrefAMux_Select((uint8)config);
        #endif

        ADC_4_currentConfig = config;
...
/*******************************************************************************
* Function Name: ADC_4_StartConvert
****************************************************************************//**
*
...
* overridden at run time with the ADC_4_SetConvertMode() function.
*
* \param None
*
* \return None
...
{
    if (SAR_SAMPLE_CTRL_DSI_TRIGGER_LEVEL_Msk == (ADC_4_SAR__HW->SAMPLE_CTRL & SAR_SAMPLE_CTRL_DSI_TRIGGER_LEVEL_Msk))
    {
        Cy_SAR_StartConvert(ADC_4_SAR__HW, CY_SAR_START_CONVERT_CONTINUOUS);
    }
    else
    {
        Cy_SAR_StartConvert(ADC_4_SAR__HW, CY_SAR_START_CONVERT_SINGLE_SHOT);
    }
}

/*******************************************************************************
* Function Name: ADC_4_SetConvertMode
****************************************************************************//**
*
...
void ADC_4_SetConvertMode(cy_en_sar_start_convert_sel_t mode)
{
    ...
    case CY_SAR_START_CONVERT_CONTINUOUS:
        ADC_4_SAR__HW->SAMPLE_CTRL |= SAR_SAMPLE_CTRL_DSI_TRIGGER_LEVEL_Msk;
        break;
...
/* ****************************************************************************
* Function Name: ADC_4_IRQ_Enable
****************************************************************************//*
*
* \brief Enables interrupts to occur at the end of a conversion. Global
* interrupts must also be enabled for the ADC_4 interrupts to occur.
*
* \param None
*
* \return None
...
/* ****************************************************************************
* Function Name: ADC_4_IRQ_Disable
****************************************************************************//*
*
...
void ADC_4_IRQ_Disable(void){
    ...
    #ifdef ADC_4_IRQ__INTC_CORTEXM4_ASSIGNED
    #if (CY_CPU_CORTEX_M4)
        NVIC_DisableIRQ(ADC_4_IRQ_cfg.intrSrc);
    #endif
    #endif
...
/*******************************************************************************
* Function Name: ADC_4_SetEosMask
****************************************************************************//**
*
...
void ADC_4_SetEosMask(uint32_t mask)
{
    ...
    Cy_SAR_SetInterruptMask(ADC_4_SAR__HW, intrMaskReg);
}

/******************************************************************************
* Function Name: ADC_4_SetChanMask
****************************************************************************//*
*
...
{
    uint32 chanCount = ADC_4_allConfigs[ADC_4_currentConfig].numChannels;
    enableMask &= (uint32)((uint32)(1ul << chanCount) - 1ul);

    Cy_SAR_SetChanMask(ADC_4_SAR__HW, enableMask);
}

/*******************************************************************************
* Function Name: ADC_4_IsEndConversion
****************************************************************************//**
*
...
* If the returned value is zero, the ADC_4 is still calculating the last result.
*
...
uint32_t ADC_4_IsEndConversion(cy_en_sar_return_mode_t retMode)
{
    ...
    result = Cy_SAR_IsEndConversion(ADC_4_SAR__HW, retMode);
...
void ADC_4_Init(void)
{
    ...
    if(0u == ADC_4_initVar)
    {
        ...
        #ifdef ADC_4_IRQ__INTC_CORTEXM4_ASSIGNED
        #if (CY_CPU_CORTEX_M4)
            (void)Cy_SysInt_Init(&ADC_4_IRQ_cfg, &ADC_4_ISR);
        #endif
        #endif
        ...
        if(0u == ADC_4_selected)
        {
            ...
            ADC_4_InitConfig(&ADC_4_allConfigs[configNum]);

            /* Change Vref selection if is was routed by Creator. Make. */
            #if (ADC_4_VREF_ROUTED)
                ADC_4_vrefAMux_Select((uint8)configNum);
            #endif
        }
    }
}
